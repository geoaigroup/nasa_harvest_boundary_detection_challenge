...
def create_indices(L,L1,dim=0,max_L=None):
    mv = (L-1)/2

    indices = (torch.arange(start=0,end=L,step=1,requires_grad=False) - mv)
    indices = indices.unsqueeze(dim).repeat_interleave(repeats=L1,dim=dim).unsqueeze(0)
    ...
    if max_L is None:
        ...
        max_L = (max_L-1)/2
        mask = torch.abs(indices)>max_L
        indices[mask] = -2

    indices = indices / max_L
    return indices,mask

def add_coords(data,max_L=None):
    B,C,H,W = data.shape
    
    x,mask_x = create_indices(W,H,dim=0,max_L=max_L)
    y,mask_y = create_indices(H,W,dim=1,max_L=max_L)
    ...
    coords = torch.cat([x,y,dis],dim=0).unsqueeze(0).repeat_interleave(repeats=B,dim=0).to(data.device)
    data = torch.cat([data,coords],dim=1)
...
class Unet3D(Unet):
    ...
    def forward(self,x):
        ...
        if self.coord_conv:
            decoder_output = add_coords(decoder_output,max_L=self.cc_max_L)
...
if __name__ == '__main__':
    ...
    model = Unet3D(
        tsteps = 6,
        kernel_3d = 3,
        conv3d_mode = 'conv3d_deep_pc',
        encoder_name = "tu-repvgg_b0",
        encoder_depth = 5,
        sa_att=False,
        ch_mul=4,
